...
try:
 ...
 log = open('log.txt', 'a') 
 subprocess.Popen(['benchexec', sys.argv[1],'--no-compress-results'],stdout=log, stderr=log).wait()
...
for key,value in JBMC_taskFile_Root.find('rundefinition').items():
 taskName = value
witness_File = line[line.find(' ')+1:line.rfind('results')] + 'files' + '/' + '${rundefinition_name}' + '/'+ '${taskdef_name}' + '/' + witness_FileName
...
Validator_taskFile_Root.set('tool','WitForJBMC')
...
try:
 ...
 log = open('log.txt', 'a')
 subprocess.Popen(['benchexec', '../Tasks_JBMCWitnessValidator.xml', '--no-compress-results'],stdout=log).wait()
